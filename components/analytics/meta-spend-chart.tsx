...
import type { MetaDailyRow } from '@/services/meta/ads'
...
const METRICS = [
  { key: 'spend', label: 'Spend', color: '#ec4899', axis: 'left' as const, format: (v: number) => `$${v.toFixed(2)}` },
...
export function MetaSpendChart({ data }: Props) {
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={chartData} margin={{ top: 4, right: hasRight ? 48 : 16, left: 0, bottom: 0 }}>
            <defs>
              {METRICS.map((m) => (
                <linearGradient key={m.key} id={`metaGrad_${m.key}`} x1="0" y1="0" x2="0" y2="1">
