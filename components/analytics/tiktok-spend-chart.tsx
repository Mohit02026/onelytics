...
const METRICS = [
  { key: 'spend', label: 'Spend', color: '#14b8a6', axis: 'left' as const, format: (v: number) => `$${v.toFixed(2)}` },
  { key: 'impressions', label: 'Impressions', color: '#3b82f6', axis: 'right' as const, format: (v: number) => Number(v).toLocaleString() },
  { key: 'reach', label: 'Reach', color: '#8b5cf6', axis: 'right' as const, format: (v: number) => Number(v).toLocaleString() },
  { key: 'clicks', label: 'Clicks', color: '#10b981', axis: 'right' as const, format: (v: number) => Number(v).toLocaleString() },
  { key: 'videoViews', label: 'Video Views', color: '#f59e0b', axis: 'right' as const, format: (v: number) => Number(v).toLocaleString() },
]
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={chartData} margin={{ top: 4, right: hasRight ? 48 : 16, left: 0, bottom: 0 }}>
            <defs>
              {METRICS.map((m) => (
                <linearGradient key={m.key} id={`ttGrad_${m.key}`} x1="0" y1="0" x2="0" y2="1">
