...
const METRICS = [
  ...
  { key: 'users', label: 'Users', color: '#a855f7', axis: 'left' as const },
  { key: 'pageviews', label: 'Pageviews', color: '#10b981', axis: 'right' as const },
]

export function SessionsChart({ data }: Props) {
  const [active, setActive] = useState<Set<string>>(new Set(['sessions', 'users']))
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={260}>
          <AreaChart data={chartData} margin={{ top: 4, right: hasRight ? 48 : 16, left: 0, bottom: 0 }}>
            <defs>
              {METRICS.map((m) => (
                <linearGradient key={m.key} id={`ga4Grad_${m.key}`} x1="0" y1="0" x2="0" y2="1">
            ...
            {hasLeft && (
              <YAxis yAxisId="left" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} width={44} tickFormatter={fmtNum} />
            )}
            {hasRight && (
              <YAxis yAxisId="right" orientation="right" tick={{ fontSize: 11, fill: '#9ca3af' }} tickLine={false} axisLine={false} width={44} tickFormatter={fmtNum} />
            )}
            <Tooltip
            ...
            />
            <Legend formatter={(v) => METRICS.find((m) => m.key === v)?.label ?? v} wrapperStyle={{ fontSize: '12px', paddingTop: '12px' }} />
            {activeMetrics.map((m) => (
              <Area
                key={m.key}
                yAxisId={m.axis}
