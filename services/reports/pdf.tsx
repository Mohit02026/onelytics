...
const DAILY_COLS = [
  { id: 'google', label: 'Google Ads', platform: 'googleAds' },
  { id: 'meta', label: 'Meta Ads', platform: 'meta' },
  { id: 'tiktok', label: 'TikTok Ads', platform: 'tiktok' },
  { id: 'linkedin', label: 'LinkedIn Ads', platform: 'linkedin' },
]
...
export const PdfDocument = ({ data, title, startDate, endDate, createdAt }: { data: ReportData, title: string, startDate: string, endDate: string, createdAt: Date }) => {
  ...
  const sel = data.selectedPlatforms ?? DAILY_COLS.map((c) => c.platform)
  const activeDailyCols = DAILY_COLS.filter((c) => sel.includes(c.platform))
  ...
  return (
    <Document>
      ...
      {/* Daily Spend */}
      {data.dailySpend.length > 0 && activeDailyCols.length > 0 && (
        <Page style={styles.page}>
          <View style={styles.sectionHeader}><Text style={styles.sectionHeaderText}>Daily Spend by Channel</Text></View>
          <DataTable
            headers={[
              { label: 'Date', w: '20%' },
              ...activeDailyCols.map((c) => ({ label: c.label, w: `${80 / activeDailyCols.length}%` })),
            ]}
            rows={data.dailySpend.map((d) => [
              String(d.date),
              ...activeDailyCols.map((c) => fmt$(d[c.id] ?? 0)),
            ])}
          />
          <Text style={styles.footer} fixed>Generated by Onelytics · {new Date(createdAt).toLocaleDateString()}</Text>
        </Page>
